...
4. ✅ 从产出文件反推环境值（第三条）—— 引擎
   `recipe_import.solve_env_from_file`（同一台解算器，换一个落点），编排
   `core/env_import.py`，GUI `widgets/env_import_dialog.py`，
   CLI `auto-ext profile read-env`。**红区上 CLI 才是常用的那个**：
   X11 跨大陆每次重绘一个来回，而这是个一屏就答完的问题。

   两条不肯让步的规则：**不写盘**（采纳一个站点事实是决定，不是操作结果 ——
   GUI 那边走 `apply_edit` 变成暂存编辑，仍然要按 Save；CLI 要显式 `--write`），
   以及**不替用户裁决**：两份文件对同一个变量给出不同答案时，两个答案都摆出来、
   都不预勾、`--write` 也跳过它。投票或者按顺序取第一个都是在发明本项目没有的规则，
   而错的那个答案和对的那个一样像真的。
